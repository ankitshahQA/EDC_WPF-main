...
public class ConfigControl2 extends JFrame {
	...
	public static JCheckBox loginportal, loginmanager, loginserver, groupfunctionality, webappbuilder, organization,
			scenelayer, feature, tile, map, content, ServerRole;
...
//variables to read values from UI and store in config file	
	...
	public static boolean flagServerRole = true;
	...
	private static void createAndShowGUI() {
		...
		frame = new ConfigControl2();
		((ConfigControl2) frame).initialize1();
	...
	private void initialize1() {
		...
		try {
			...
			ServerRole = new JCheckBox("Validate Server Roles");
			ServerRole.setBackground(Color.GRAY);
			ServerRole.setBorderPainted(false);
			ServerRole.setFont(new Font("Verdana", Font.PLAIN, 14));
				...
				public void itemStateChanged(ItemEvent e) {
					...
					if (e.getStateChange() == ItemEvent.DESELECTED) {
						...
						if (tile.isSelected() || feature.isSelected() || groupfunctionality.isSelected()
								...
								|| content.isSelected() || ServerRole.isSelected()) {
							loginportal.setEnabled(false);
						} else {
			...
			ServerRole.addItemListener(new ItemListener() {
			...
			content.addItemListener(new ItemListener() {
				...
				public void itemStateChanged(ItemEvent e) {
					...
					if (e.getStateChange() == ItemEvent.DESELECTED) {
						...
						if (tile.isSelected() || feature.isSelected() || groupfunctionality.isSelected()
								|| scenelayer.isSelected() || organization.isSelected() || map.isSelected()
								|| webappbuilder.isSelected() || ServerRole.isSelected()) {
							loginportal.setEnabled(false);
						} else {
							loginportal.setEnabled(true);
						}
				...
				public void itemStateChanged(ItemEvent e) {
					...
					if (e.getStateChange() == ItemEvent.DESELECTED) {
						...
						if (tile.isSelected() || feature.isSelected() || groupfunctionality.isSelected()
								...
								|| scenelayer.isSelected() || ServerRole.isSelected()) {
							loginportal.setEnabled(false);
						} else {
							loginportal.setEnabled(true);
						}
	...
	static void selecttestfunctionality() {
		try {
			...
			layout.getConstraints(ServerRole).setHeight(Spring.constant(h2));
			...
			layout.putConstraint(SpringLayout.NORTH, ServerRole, distance, SpringLayout.SOUTH, loginserver);
			layout.putConstraint(SpringLayout.WEST, ServerRole, textstart, SpringLayout.WEST, panel);

			layout.putConstraint(SpringLayout.NORTH, content, distance, SpringLayout.SOUTH, ServerRole);
